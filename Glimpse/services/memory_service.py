...
class MemoryService:
    ...
    def create_cluster_memory_async(
        ...
        """Create a memory from multiple screenshots asynchronously."""
        if not self._task_queue:
            if on_error:
                on_error("Task queue not configured")
            return
        ...
        def task():
            try:
                ...
                success = True
                if self._ai_client.is_configured() and combined_text:
                    ai_summary = self._ai_client.generate_summary(
                        combined_text,
                        prompt="以下内容来自一组连续截图，请生成一个综合摘要："
                    )
                else:
                    ai_summary = combined_text[:200] if combined_text else "无内容"
                ...
                # Store extra images as JSON
                if len(image_paths) > 1:
                    import json
                    record.extra_images = json.dumps(image_paths[1:])

                if not self._sqlite_manager.insert_memory(record):
                    if on_error:
                        on_error("Failed to save cluster memory")
                    return

                # Embed and store in Chroma
                embedding_text = f"{ai_summary} {combined_text}".strip()
                if embedding_text:
                    embedding = self._embedding_client.get_embedding(embedding_text)
                    if embedding:
                        self._chroma_manager.add_memory(
                            memory_id=memory_id,
                            text=embedding_text,
                            embedding=embedding,
                            metadata={"app_name": app_name, "created_at": created_at},
                        )
