...
正文使用 Segoe UI，并为简体中文保留 PingFang SC 和 Microsoft YaHei 回退。Windows 上的阅读速度比字体展示性更重要。
...
首页搜索区域是一块完整的工具浮层。页面左右为 20px 留白，浮层内部 padding 为 12px。搜索框、模式选择、刷新、开发工具、截图和添加记忆按钮都使用 36px 的紧凑高度，一级功能之间保持约 10px 间距。操作区与搜索模式之间保留分隔线和 12px 左侧空间。“添加记忆”使用深蓝主操作并固定在砖橙色“截图”右侧，让手动记录与画面捕捉相邻但不混淆。

搜索工具栏固定在记忆窗格顶部，并使用全局 `--z-sticky: 100` 层级。记忆窗格是首页左侧唯一的垂直滚动容器；数量栏、分组标题和记忆卡片随内容一起向上滚动，并延伸到浮栏背后、应用头下方。浮栏外层保持透明，只在控件表面背后使用一层向下淡出的半透明页面色遮罩；独立的 4px 背景模糊层通过渐变蒙版由模糊过渡到清晰，避免矩形边界。浮栏主体保持不透明，不重复施加模糊。右侧详情面板保持独立，不共享这条滚动链。

窄屏可以换行，但不能换一套视觉语言：控件高度、组内间距、圆角和操作顺序保持不变。
...
图片记忆卡的媒体框使用约 1.4:1 的横向比例与 `object-contain`，在不裁切截图的前提下减少上下留白。

纯文本记忆使用内容优先的两段式索引卡变体：上方以 `primary-soft` 浅蓝底承载固定六行高度的正文预览，下方使用独立白色信息区，时间固定在右侧，左侧保留为未来自定义 Tag 的稳定容器；在真实 Tag 功能落地前不显示占位标签或说明文案。卡片按自身内容高度排列，不被同一行的截图卡拉伸；正文区域保持对称的左右内边距，不为平台或选中图标预留额外空间。不使用类型标签、文档图标、彩色竖线或“手动添加”等重复元数据。内容类型只作为数据属性和筛选条件存在。它不是“图片加载失败”的占位状态；详情与检查器也不展示类型标签、媒体或 OCR 模块，只保留正文、索引状态、复制、编辑和管理操作。

### 筛选面板

筛选是可扩展的工具面板。当前提供“时间范围”和“内容类型”两组条件；内容类型使用“图片”“文本”两个方形复选框，未勾选时不限制类型，单独或同时勾选时按任一匹配，并同时作用于浏览与搜索结果。后续来源渠道等维度沿用同一分组结构：分组标题和纵向选项列表。分组之间只用垂直留白建立层级，不绘制穿过标题的横线。未落地的筛选能力不提前显示为假控件。
...
单选按钮使用 16px 圆形：未选中时为中性描边，选中时为完整蓝色圆面与 5px 白色中心点。多选按钮使用 16px 方形与白色勾选标记，沿用相同的行高、文字间距和焦点状态。底部操作区用一像素分割线固定分区，“应用筛选”与“重置”两个按钮等宽、保持 40px 高度并使用 6px 圆角。该密度参考桌面属性面板：直观、可扫读，不做表单卡片化装饰。
