...
纯文本记忆的界面行为也属于稳定产品边界：记忆墙使用浅蓝正文区（固定六行预览）
和白色信息区，时间固定在右侧，左侧保留未来自定义 Tag 的位置；在 Tag 功能正式
落地前不显示占位标签。卡片、详情页和侧边检查器不显示类型标签、文档图标、彩色
竖线、图片或 OCR 模块。打开侧边检查器后的选中态只使用卡片轮廓表达，不叠加勾选
图标；正文左右内边距保持一致。内容类型筛选使用“图片”“文本”两个方形多选框：
未勾选表示不限，单独或同时勾选按任一类型匹配。具体视觉 token 和布局数值以根目录
`DESIGN.md` 为准。
